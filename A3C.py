...
class Worker(mp.Process):
    def __init__(self, env_name, rank, globalNet, localNet, optimizer, totalEpisode, globalReturn, Q, params):
    ...
    def run(self):
        ...
        while self.totEps.value < self.params['MAX_EPISODE']:
            ...
            for t in range(self.params['MAX_STEP']):
                # if self.rank == 0:
                #     self.env.render()
                action = self.LNet.selectAction(torch.from_numpy(
                    state.reshape(1, -1).astype(np.float32)).to(device))
                ...
                if steps % self.params['UPDATE_STRIDE'] == 0 or done:
                    loss = self.learn(
                        nextState, done, stateBuf, actionBuf, rewardBuf)
                    stateBuf, actionBuf, rewardBuf = [], [], []
                    ...
                    if done:
                        ...
                        print("Rank: %d\tEps: %d\tTotRet: %f\tSteps: %d\tLoss: %f" % (
                            self.rank, self.totEps.value, self.totR.value, t + 1, loss))
    ...
    def learn(self, nextState, done, stateBuf, actionBuf, rewardBuf):
        if done:
            ...
            R = self.LNet.forward(torch.from_numpy(nextState.reshape(
                1, -1).astype(np.float32)).to(device))[-1][0].item()
        ...
        loss = self.LNet.loss(
            torch.from_numpy(np.vstack(stateBuf).astype(
                np.float32)).to(device),
            torch.from_numpy(np.vstack(actionBuf).astype(
                np.float32)).to(device),
            torch.from_numpy(
                np.array(RBuf).reshape(-1, 1).astype(np.float32)).to(device)
        )
        self.opt.zero_grad()
        loss.backward()
...
class A3C:
    def __init__(self, env_name, gamma, updateStride, maxEps, maxSteps, hiddenSize, lr):
        self.params = {'MAX_EPISODE': maxEps, 'MAX_STEP': maxSteps,
                       'UPDATE_STRIDE': updateStride, 'gamma': gamma, 'hiddenSize': hiddenSize}
        self.env = gym.make(env_name).unwrapped
        self.globalNet = ACNet(
            self.env.observation_space.shape[0], hiddenSize, self.env.action_space.shape[0]).to(device)
        self.globalNet.share_memory()
        self.opt = SharedAdam(self.globalNet.parameters(),
    ...
    def train(self):
        workers = [Worker(self.env_name, rank, self.globalNet,
                          ACNet(self.env.observation_space.shape[0], self.params['hiddenSize'], self.env.action_space.shape[0]).to(
                              device),
                          self.opt,
                          self.totEps,
                          self.totR,
