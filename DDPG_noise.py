        ...
        while len(self.buffer) < initLen:
            ...
            for t in count():
                ...
                self.push(torch.tensor(state.reshape(1, stateSize), device=device, dtype=torch.float),
                          ...
                          torch.tensor(np.array([reward]).reshape(
                              1, -1), device=device, dtype=torch.float),
                          torch.tensor(nextState.reshape(
                              1, stateSize), device=device, dtype=torch.float),
...
class DDPG:
    def __init__(self, env, buf, actorLR, criticLR, gamma, tau, batchSize, hiddenSize, maxSteps, maxEps, updateStride):
        ...
        self.param_noise = AdaptiveParamNoiseSpec(
            initial_stddev=0.05, desired_action_stddev=0.2, adaptation_coefficient=1.01)
        self.updateStride = updateStride

        self.actor = Actor(self.env.observation_space.shape[0], self.hiddenSize,
                           self.env.action_space.shape[0], self.env.action_space.high[0]).to(device)
        self.actor_perturbed = Actor(
            self.env.observation_space.shape[0], self.hiddenSize, self.env.action_space.shape[0], self.env.action_space.high[0]).to(device)
        self.actorTarget = Actor(
            self.env.observation_space.shape[0], self.hiddenSize, self.env.action_space.shape[0], self.env.action_space.high[0]).to(device)
        self.critic = Critic(
            self.env.observation_space.shape[0], self.hiddenSize, self.env.action_space.shape[0]).to(device)
    ...
    def _getAction(self, state, isTensor=False, isPerturbed=True):
        if isTensor:
            if isPerturbed:
                action = self.actor_perturbed.forward(state).detach() + torch.tensor(
                    self.noise.sample() * self.env.action_space.high[0], device=device, dtype=torch.float)
            else:
                action = self.actor.forward(state).detach()
        else:
            if isPerturbed:
                action = self.actor_perturbed.forward(torch.tensor(state.reshape(-1, self.env.observation_space.shape[0]), device=device, dtype=torch.float)).detach(
                ) + torch.tensor(self.noise.sample() * self.env.action_space.high[0], device=device, dtype=torch.float)
            else:
                action = self.actor.forward(torch.tensor(
                    state.reshape(-1, self.env.observation_space.shape[0]), device=device, dtype=torch.float)).detach()
    ...
    def adapt(self, noise_counter):
        if self.buffer.position - noise_counter > 0:
            noise_data = self.buffer.buffer[self.buffer.position -
                                            noise_counter: self.buffer.position]
        else:
            noise_data = self.buffer.buffer[self.buffer.position - noise_counter +
                                            self.buffer.capacity: self.buffer.capacity] + self.buffer.buffer[0: self.buffer.position]
        ...
        unperturbed_actions = self._getAction(
            noise_s, isTensor=True, isPerturbed=False)
        dist = self.ddpg_distance_metric(
    ...
    def learn(self):
        ...
        targetQ = self.criticTarget.forward(
            nextStateBatch, nextActionBatch).detach().view(-1)
