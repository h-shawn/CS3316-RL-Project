...
class Agent:
    def __init__(self, actionSpace, observationSpace, gamma, epsilon, tau, batchSize, lr, hiddenSize, learnStride, updateStride, maxStep, isDueling=False):
        ...
        if isDueling:
            self.net = DuelingDQN(
                84, 84, 4, self.hiddenSize, self.actionNum).to(device)
            self.targetNet = DuelingDQN(
                84, 84, 4, self.hiddenSize, self.actionNum).to(device)
        else:
            self.net = DQN(84, 84, 4, self.hiddenSize,
                           self.actionNum).to(device)
            self.targetNet = DQN(84, 84, 4, self.hiddenSize,
                                 self.actionNum).to(device)
    ...
    def learn(self, buf, algo):
        ...
        if algo == 'DQN':
            ...
            targetAction = self.net(nextStateBatch).max(
                1)[1].view(-1, 1)  # get argmax Q'(s_t+1, a)
            ...
            for t in count():
                ...
                buf.push(resize(state).unsqueeze(0).to("cpu"),
                         ...
                         torch.tensor(np.array([reward]).reshape(
                             1, -1), device="cpu", dtype=torch.float),
                ...
                if done or t + 1 >= self.maxStep:
                    ...
                    print("Episode %d ended after %d timesteps with score %f, buf=%d, epsilon=%f in %fs" % (
                        i + 1, t + 1, score, len(buf), self.epsilon, endingTime-startTime))
                    break
            if i % 1000 == 0:
                ...
                torch.save(self.targetNet.state_dict(),
                           'targetNet' + str(i) + '.pkl')
        ...
        while len(self.buffer) < initLen:
            ...
            for t in count():
                ...
                self.push(resize(state).unsqueeze(0).to("cpu"),
                          torch.tensor(np.array([action]).reshape(
...
class DuelingDQN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = val + adv - \
            adv.mean(1).unsqueeze(1).expand(x.size(0), self.outputSize)
        return x
